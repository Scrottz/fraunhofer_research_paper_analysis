...
![Figure 1. The FineInstructions pipeline for efficiently generating diverse, pre-training scale, synthetic instruction-answer pairs.](no_doc/assets/2601.22146v1/figure_2.png)
...
- Collection of human written prompt templates from various libraries
    - Harmful Query Filtering: OpenAI Moderation API (Markov et al., 2023) removes toxic/unsafe content
    - Decontamination: Tulu 3 procedure (Lambert et al., 2024a) prevents benchmark leakage by filtering queries overlapping with evaluation datasets (MixEval, MT-Bench-101, AlpacaEval)
    - Result: ~18M clean, diverse, decontaminated instruction templates
- Convert queries to templates by inserting <fi></fi> tags for entities or scenarios
...
- ~50k queries were selected for the fine tuning of a Llama-3.2 1B Instruction model
    - Templates were converted to "silver" synthetic data by prompting Llama-3.3 70B Instruct with task-specific prompts (detailed in Appendix C)
    - Prompts instruct the LLM to:
        1. Genericize queries by inserting `<fi></fi>` tags around entities/scenarios
        2. Generate "compatible document descriptions" (short paragraphs describing documents that could instantiate the template and provide answers)
...
- Mathematical Formulation:
    - Gaussian weights: \( w_{k,t} = \frac{m_t \exp\left(-\frac{1}{2}\left(\frac{t - c_k}{\sigma T}\right)^2\right)}{\sum_{t'} m_{t'} \exp\left(-\frac{1}{2}\left(\frac{t' - c_k}{\sigma T}\right)^2\right)} \)
    - Chunk centers: \( c_k = \rho_k T \), where \( \rho_k = \frac{k}{K+1} \)
    - Blending parameter: α=1.0 (pure local embeddings, no global mixing)
    - Width parameter: σ=0.05
- Two-Stage Fine-Tuning:
    - Round 1: Train on hard positives/negatives for document-template compatibility
    - Round 2: Train with chunk-index labels to reduce representation mixing across local embeddings
    - Goal: Maintain distinct semantic representations for each of the K=5 document chunks
...
- Compute Optimization (Tag-based Copying)
    - Problem: Token-by-token generation of long sequences causes:
        - High computational cost (autoregressive decoding)
        - GPU memory bottlenecks (possible during generation stage)
    - Solution: The model generates short XML-like tags with ellipsis notation
        - Example: "It is known that no preferred inertial frame exists according to the principle of relativity" is generated as `<excerpt>It is known that <...> the principle of relativity.</excerpt>`
        - Model generates opening tag, ellipsis marker `<...>`, and closing tag instead of full text
    - Post-Processing: Tags are programmatically expanded by copying exact text spans from source document
    - Impact: Reduces the number of generated tokens needed to excerpt long spans of text, enabling scaling to 1B+ instruction-answer pairs
...
- Quality Control & Metrics
    - Model: Flow Judge (distilled 3.8B parameter judge model, AI 2024a)
    - Evaluation: 5-point Likert scale rubric:
        - 1 = Irrelevant or off-topic
        - 5 = Addresses the query without extraneous, vague, or repetitive content
    - Filter Threshold: Retain only pairs scoring ≥4
    - Ablation Result: Judging stage improves performance on benchmarks, specifically AlpacaEval (see Appendix E)
...
- Data Budgeting and Selection
    ...
    - Selection Pipeline
        ...
        - Accumulation logic: Pairs are randomly selected until the next addition would exceed the token limit
        - Discard Rate: Strict token budget enforcement results in ~50% of generated pairs being discarded
        - Average Yield: ~3 instruction-answer pairs retained per document (50% of the 6 generated)
- Format and Structural Design
    - Template Structure: Chat template "Instruction: {{instruction}}\n\nAnswer: {{answer}}"
    - Standalone vs Contextual:
        - Baselines (IPT, Nemotron-CC): Q&A pairs appended to raw documents (reading-comprehension style, requires document as context)
        - FineInstructions: Standalone Q&A pairs (context-independent, document content integrated into answer)
    - Standalone Advantages:
        - Eliminates redundant context and forces direct assistant alignment
        - Brings pre-training data further in-distribution with expected downstream usage by users
...
![Table 1. Benchmark performance of 1.8B parameter models pre-trained on FineInstructions and various baselines. We bold the strongest method and bold the win rate % if FineInstructions wins.](no_doc/assets/2601.22146v1/table_1.png)

![Figure 2. Efficiency from pre-training on FineInstructions data](no_doc/assets/2601.22146v1/figure_1.png)
...
![Figure 3. A visualization of the task diversity in FineInstructions.](doc/assests/2601.22146v1/all_sunburst.png)
...
- Benchmark limitations: 
    - Current LLM benchmarks focus on factual recall and are ill-suited for long-tail realistic knowledge tasks (advice, suggestions, recommendations)
    - Paucity of benchmarks for open-ended, subjective tasks that reflect real-world LLM usage
- Evaluation methodology constraints:
    - Log-probability-based evaluation (e.g., multiple-choice scoring) is unreliable for instruction-tuned models (Wang et al., 2024; Chandak et al., 2025)
    - Models trained on standalone Q&A assign low probability to short-form answers, biasing toward long-form responses
    - Solution: Extractive or LLM-as-a-judge evaluation (used in this work) is more appropriate
...
- The Token Tax and Information Loss
    - Framing questions and formatting templates consumes token budget
